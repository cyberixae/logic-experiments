...
export const isCRResult: Refinement<AnySequent, AnyCRResult> = refineActiveR(isConjunction);
...
> = Transformation<
  R, [Derivation<Sequent<Γ, [A, ...Δ]>>, Derivation<Sequent<Γ, [B, ...Δ]>>], 'cr'
>;
...
export type ApplyCR<S1 extends AnyDerivation, S2 extends AnyDerivation> = [
  ...
  S2
    ...
    >
  ] ? CR<Γ, A, B, Δ, CRResult<Γ, A, B, Δ>> : never;
