...
const precedence = (p: prop.Prop): number =>
  prop.matchRaw(p, {
    atom: () => 4,
    falsum: () => 4,
    verum: () => 4,
...
const expand = (minPrec: number, operand: prop.Prop): Printer => {
  if (operand.kind === 'atom') return fromProp(operand)
  return precedence(operand) >= minPrec
    ? print('optional')(fromProp(operand))
    : print('parenthesis')(fromProp(operand))
}
  ...
  return (t) => [
    segment.of(
      rule.matchRuleId(s, {
        ...
        tswp: () => 'W' + atomChar('p'),
        tswq: () => 'W' + atomChar('q'),
        tswpp: () => 'W' + atomChar('p') + atomChar('p'),
        tswpq: () => 'W' + atomChar('p') + atomChar('q'),
        tswqp: () => 'W' + atomChar('q') + atomChar('p'),
        tswqq: () => 'W' + atomChar('q') + atomChar('q'),
        tc: () => t.conjunction.join(empty),
        td: () => t.disjunction.join(empty),
      }),
    ),
  ]
}
